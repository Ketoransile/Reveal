...
export default function DashboardLayout({
    ...
    return (
        <div className="flex min-h-screen bg-background relative isolate">
            ...
            <main
                className={`flex-1 flex flex-col min-h-screen transition-all duration-300 ease-in-out ${collapsed ? 'md:ml-20' : 'md:ml-64'}`}
            >
                <SiteHeaderCustom onMenuClick={() => setMobileOpen(true)} />
                <div className="flex-1 p-6 lg:p-10 w-full max-w-7xl mx-auto space-y-4">
                    {children}
                </div>
            </main>
        </div>
